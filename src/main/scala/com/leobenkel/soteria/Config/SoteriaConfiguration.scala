...
private[soteria] case class SoteriaConfiguration(
  ...
  @transient lazy private val RawModules: Seq[(Dependency, Seq[String])] =
    modules
      .flatMap {
        case (org, mm) => mm.map { case (name, m) => m.toDependency(org, name, retrieveModule) }
      }
      .toSeq

  @transient lazy val AllModules: Seq[Dependency] =
    RawModules.map {
      case (module, errors) =>
        if(errors.nonEmpty) {
  ...
  @transient lazy val ForbiddenModules:             Seq[(Dependency, String)]          =
    AllModules.filter(_.isForbidden).map(m => (m, m.forbiddenExplanation))
    ...
    (log: LoggerExtended) =>
      (input: Map[String, Any]) => {
        for {
          ...
          _            <-
            if(scalaVersion.isEmpty) Left("'scalaVersions' cannot be empty")
            else Right(())
