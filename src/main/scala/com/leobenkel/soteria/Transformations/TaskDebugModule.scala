...
private[Transformations] trait TaskDebugModule {
  ...
  @transient lazy private val parseModule: Parser[Dependency] = {
    ...
    (OptSpace ~> orgParser ~
      ...
      (singlePercent ~> revisionParser) <~ OptSpace).map {
      case ((organization, (percent, artifact)), revision) => Dependency(
          organization,
          artifact,
          revision,
  ...
  def debugModuleCommand: Command = {
    Command.args(CommandName, "") { (state, args) =>
  ...
  def debugAllModuleCommand: Command = {
    Command.args(CommandNameAll, "") { (state, _) =>
      val log           = Project.extract(state).get(SoteriaPluginKeys.soteriaGetLog)
      ...
      val (_, fullConfig) =
        allScalaVersions.foldLeft((state, config)) {
          case ((currentState, conf), scalaVersion) =>
            ...
            allModules.foldLeft((currentState, conf)) {
              case ((currentState, conf), module) => executeCompilation(
                  state = currentState,
                  scalaVersion = scalaVersion,
                  config = conf,
                  m = module,
  ...
  object ZTestOnlyTaskDebugModule {
    ...
    def execute(
        ...
        module:  Either[String, Dependency],
        execute: ModuleID => SoteriaConfiguration,
    ): Option[SoteriaConfiguration] = executeDebugModuleCommand(log, module, execute)
  }
}
