...
private[soteria] object SoteriaPluginKeys {
  ...
  lazy val soteriaAssemblySettings =
    taskKey[AssemblyOption](
      "Use to set the Assemble option to the right values"
    )

  lazy val soteriaGetAllDependencies =
  ...
  lazy val soteriaSoftOnCompilerWarning =
    settingKey[Boolean](
  ...
  // coverall
  lazy val soteriaTestCoverage:   FancySettings[TaskKey[Unit]] =
    FancySettings(
      setting = taskKey[Unit]("PRIVATE - Run test coverage"),
      nameAsString = "soteriaTestCoverage",
    )
  lazy val soteriaSubmitCoverage: FancySettings[TaskKey[Unit]] =
    FancySettings(
