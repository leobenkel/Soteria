...
case class Dependency(
  ...
  @transient lazy val needToBeReplaced
    : Boolean = !overrideIsEnough && exclusionRule.isRight
  ...
  @transient lazy val toSerializedModule: SerializedModule = SerializedModule(
    ...
    scalaVersionsFilter =
      ModuleDefaults.toOption(this.scalaVersionsFilter.map(_.serialized))
  )
  ...
  def withName(f: NameOfModule => NameOfModule): Dependency =
    this.copy(nameObj = f(this.nameObj))

  def withVersion(version: String): Dependency =
    this.copy(versions = Set(version))

  private def shouldBeTestedForInclusion(m: ModuleID): Boolean = {
    ((m.name.contains("_") || m.crossVersion
  ...
  def shouldBeDownloaded(scalaV: ScalaV): Boolean = {
    this.toModuleID.right
      .map(shouldBeDownloaded(scalaV, _))
      .right
      .getOrElse(false)
  }
