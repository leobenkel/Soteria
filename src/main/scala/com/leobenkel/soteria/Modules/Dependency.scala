...
case class Dependency(
  ...
  @transient lazy val versionKey:                 String                                     =
    if(versions.isEmpty) SerializedModule.DefaultVersionString
    else versions.mkString(" | ")
  @transient lazy val version:                    Either[String, String]                     =
    if(versions.size == 1) Right(versions.head)
    else Left(s"Expect to have one version but had ${versions.size}")
  ...
  @transient lazy val jsonKey:                    String                                     =
    if(versions.nonEmpty) s"$nameObj % ${versions.map(v => s""" "$v" """.trim).mkString(" | ")}"
    else s"$nameObj"
  @transient lazy val toSerializedModule:         SerializedModule                           =
    SerializedModule(
      version = version.fold(_ => SerializedModule.DefaultVersionString, identity),
      exactName = ModuleDefaults.toOptionWithDefault(ModuleDefaults.ExactName, nameObj.exactName),
      excludeName = ModuleDefaults.toOption(nameObj.excludeName),
      needDoublePercent =
        ModuleDefaults.toOptionWithDefault(
          ModuleDefaults.NeedDoublePercent,
          nameObj.needDoublePercent,
        ),
      shouldDownload =
        ModuleDefaults.toOptionWithDefault(ModuleDefaults.ShouldDownload, this.shouldDownload),
      overrideIsEnough =
        ModuleDefaults.toOptionWithDefault(
          ModuleDefaults.OverrideIsEnough,
          this.overrideIsEnough,
        ),
      forbidden = this.forbidden,
      shouldBeProvided =
        ModuleDefaults.toOptionWithDefault(
          ModuleDefaults.ShouldBeProvided,
          this.shouldBeProvided,
        ),
      dependenciesToRemove =
        ModuleDefaults.toOption(this.dependenciesToRemove.map(_.toPath).sorted),
      scalaVersionsFilter = ModuleDefaults.toOption(this.scalaVersionsFilter.map(_.serialized)),
    )
  @transient lazy val toConfig:                   Map[String, Map[String, SerializedModule]] =
    Map(
      this.organization ->
        Map(
  ...
  def |+|(other: Option[Dependency]): Either[String, Dependency] =
    other.map(this |+| _).getOrElse(Right(this))
...
object Dependency {
  ...
  def apply(input: (String, String)): Dependency = apply(input._1, input._2)
