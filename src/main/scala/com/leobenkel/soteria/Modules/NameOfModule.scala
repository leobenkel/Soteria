...
case class NameOfModule(
  ...
  def nameMatch(otherName: String): Boolean =
    if(exactName) otherName.toLowerCase == lowerCaseName
    else isExcludedName(otherName) &&
    otherName
      .toLowerCase
      .startsWith(
        lowerCaseName
  ...
  @transient lazy val toOrganizationArtifactName: Either[String, Dependency.OrgArtifact] =
    if(exactName)
      if(needDoublePercent) Right(organization %% name)
      else Right(organization                   % name)
    else Left(
  ...
  @transient lazy val exclusionRule: Either[String, sbt.ExclusionRule] =
    if(exactName) Right(ExclusionRule(organization = this.organization, name = this.name))
    else Left(
