...
case class SoteriaLogger(
  ...
  object TestOnly {
    ...
    @inline def prependHeaderTest(message: => String): String =
  ...
  override def setLevel(level: Level.Value): SoteriaLogger =
    this.copy(level = level)

  private def atLevel(level: Level.Value): Boolean = level.id >= getLevel.id

  private def prependHeader(message: => String): String =
    s"[${SoteriaLogger.Header}] $message"
  ...
  override def fail(message: => String): Unit =
    super.fail(prependHeader(message))
  ...
  private def makeSeparator(title: String): String = {
    ...
    val LeftForEdge = Math
      .ceil((SoteriaLogger.SeparatorLength - goodTitle.length - 4) / 2)
      .toInt
  ...
  override def setSoftError(softError: Boolean): SoteriaLogger =
    this.copy(softError = softError)
}
